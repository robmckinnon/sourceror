defmodule VendoredSourceror.Patch do
  @moduledoc """
  Functions that generate patches for common operations.

  Functions in this module assume that the AST was parsed using VendoredSourceror
  ...
  Renames a qualified or unqualified function call.
      ...
      iex> ast = VendoredSourceror.parse_string!(original)
      iex> patches = VendoredSourceror.Patch.rename_call(ast, :to_existing_atom)
      iex> VendoredSourceror.patch_string(original, patches)
  ...
  If the call is a sigil, you only need to provide the replacement letter:
      ...
      iex> original = "~H(foo)"
      iex> ast = VendoredSourceror.parse_string!(original)
      iex> patches = VendoredSourceror.Patch.rename_call(ast, :F)
      iex> VendoredSourceror.patch_string(original, patches)
      "~F(foo)"
  """
  ...
  ## Examples
      ...
      iex> original = "foo"
      iex> ast = VendoredSourceror.parse_string!(original)
      iex> patches = VendoredSourceror.Patch.rename_identifier(ast, :bar)
      iex> VendoredSourceror.patch_string(original, patches)
      "bar"
  """
  @spec rename_identifier(identifier :: Macro.t(), new_name :: atom | String.t()) :: [
          VendoredSourceror.patch()
        ]
  ...
  ## Examples
      ...
      iex> ast = VendoredSourceror.parse_string!(original)
      iex> patches = VendoredSourceror.Patch.rename_kw_keys(ast, a: :foo, e: :bar)
      iex> VendoredSourceror.patch_string(original, patches)
      "[foo: b, c: d, bar: f]"
  """
